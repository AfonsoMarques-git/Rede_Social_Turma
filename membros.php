...
if (isset($_GET['adiciona'])) // adicionar como amigo na tabela
{
    ...
    for ($j = 0; $j < $num; $j++) {
        ...
        $t2 = $result1->num_rows;
        if (($t1 + $t2) > 1)
            echo "&harr; é amigo mútuo";
        elseif (($t1) > 1)
            echo "&larr; você está a seguir";
        elseif (($t1) > 1) {
            echo "&rarr; está a segui-lo";
            $seguir = "retribuir";
        }
        if (!$t1)
            echo "[<a data-transition='slide' href='membros.php?adiciona=" . $row['utilizador'] . "'>$seguir</a>]";
        else
            echo "[<a data-transition='slide' href='membros.php?remove=" . $row['utilizador'] . "'>Não seguir</a>]";
    }
}
?>
