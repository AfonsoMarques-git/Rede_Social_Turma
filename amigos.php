<?php
    ...
    $num = $result->num_rows; // o operador de objeto, ->, é usado para acessar propriedades e métodos de um objeto
    for ($j = 0; $j < $num; $j++)
    {
        $row = $result->fetch_array(MYSQL_ASSOC); // MYSQLI_ASSOC - os itens da matriz (array) usarão o nome da coluna como uma chave de índice
    ...
    $mutuos = array_intersect($seguidores, $aseguir); // intercessão (o que é comum) de arrays para descobrir amigos mútuos
    $seguidores = array_diff($seguidores, $mutuos); // para os amigos não comuns
    $aseguir = array_diff($aseguir, $mutuos);
    $amigos = FALSE;
    echo "<br>";
    if (sizeof($mutuos)) // sizeof retorna o número de elementos de um array
    {
        echo "<span class='subhead'>$nome1 amigos mútuos</span><ul>";
