...
if ($ver != "") {
    ...
    for ($j = 0; $j < $num; ++$j) {
        ...
        if ($row['tipomsg'] == 0 || $row['autor'] == $utilizador || $row['destin'] == $utilizador) {
            echo date('M j \'y g:ia:', $row['hora']);
            echo " <a href='mensagens.php?ver=" . $row['autor'] . "'>" . $row['autor'] . "</a> ";
            if ($row['tipomsg'] == 0)
                echo "escreveu: &quot;" . $row['mensagem'] . "&quot; ";
            else
                echo "escreveu em privado: <span class='privado>&quot;" . $row['mensagem'] . "&quot;</span>";
            if ($row['destin'] = $utilizador)
                echo "[<a href='mensagens.php?ver=$ver&apaga=" . $row['id'] . "'>Apagar</a>]";
            echo "<br>";
        }
    }
}
